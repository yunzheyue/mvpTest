...
public class IMainModelIml implements IModel {

    private Handler handler = new Handler(Looper.getMainLooper());

    @Override
    public void getDataFromNet(final LoadLitener loadedLitener) {
        new Thread() {
            @Override
            public void run() {
                try {
                    ...
                    e.printStackTrace();
                    loadedLitener.onErrorListener("数据错误");
                }
                ...
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        //回调
                        loadedLitener.onLoadedLitener(data);
                    }
                });
            }
        }.start();
